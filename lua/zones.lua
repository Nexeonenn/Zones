...
function zones.FindByClass(class)
    if (#class == 0) then return zones.List end
    local tbl = {}
...
function zones.FindClosest(class, pos)
    ...
    local zone = zones.FindByClass(tostring(class))
    ...
    for id, data in pairs(zone) do
        --For each bound.
        for _, bound in pairs(data.bounds) do
            tempDist = pos:DistToSqr(bound)
    ...
    if not (target == -1) then return target, math.sqrt(dist) end
...
if SERVER then
    ...
    --removeThese is optional.
    function zones.CreatePointEnts(removeThese)
        --remove old
        for k, v in pairs(removeThese or ents.FindByClass("ent_zone_point")) do
            v:Remove()
        end
...
local function Intersect(line1, line2)
    ...
    if (sx == "-inf" or sx == "inf" or sx == "nan") then return false end
...
function zones.PointInPoly(point, poly)
    local ray = {
        x1 = point.x,
        y1 = point.y,
        x2 = 100000,
